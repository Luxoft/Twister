...
class Suts(_cptools.XMLRPCController, CommonAllocator):
    ...
    def save_sut(self, props={}, resource_name = None):
        ...
        with self.save_lock:
            ...
            logDebug("$$ {}  -- suts path {} !!".format(resource_name, filename))
    ...
    def get_sut(self, query):
        ...
        # will return result only of the SUT is in self.resources
        if "/" not in query and sutType == query:
            try:
                query = '/'.join(self.get_resource(query)['path'])
            except:
                return "*ERROR* there is no SUT having this id: {}".format(query)
        ...
        if os.path.isdir(sutPath):
            ...
            if isinstance(sutContent, dict):
                ...
                logDebug("we loaded this sut: {} and this is what we've found:\n {}".format(query, retDict))
    ...
    def create_component_sut(self, name, parent=None, props={}):
        ...
        #for testing reasons
        #resource_node = self.get_resource(parent)
        self.reserve_resource(parent, {})
        ...
        with self.acc_lock:
            ...
            logDebug("parent_p  in create component for sut: {}".format(self.reservedResources))
    ...
    def update_meta_sut(self, name, parent=None, props={}):
        ...
        #for testing reasons:
        self.reserve_resource(name, props)
        ...
        with self.acc_lock:
            ...
            logDebug("parent_p {}".format(parent_p))
    ...
    @cherrypy.expose
    def delete_component_sut(self, res_query):
        '''
    ...
    def delete_sut(self, res_query):
        ...
        def delete_sut_memory(sut_to_remove):
            logDebug(" resources {}".format(self.resources))
    ...
    @cherrypy.expose
    def import_sut_xml(self, xml_file, sutType='user'):
        '''
    ...
    @cherrypy.expose
    def export_sut_xml(self, xml_file, query):
        '''
        Export as XML file.
        '''
        user_info = self.user_info()
    ...
    @cherrypy.expose
    def save_reserved_sut_as(self, name, res_query):
        '''
    ...
    @cherrypy.expose
    def discard_release_reserved_sut(self, res_query):
