...
public class UnitTesting extends JFrame {
    ...
    public UnitTesting(String editable, String localfile, String remotefile) {
        ...
        String s = null;
        try{
            l = RunnerRepository.getLayouts().get("UTlocation").getAsString();
            s =  RunnerRepository.getLayouts().get("UTsize").getAsString();
        } catch(Exception e){
            e.printStackTrace();
        }
        if(l!=null&&s!=null){
        ...
        addWindowListener(new WindowAdapter(){
            public void windowClosing(WindowEvent e){
                go = false;
                RunnerRepository.saveUTLayout(getSize(),getLocation(),jPanel1.getDividerLocation());
                if(todelete!=null){
                    
//                     try{RunnerRepository.c.rm(todelete);}
//                     catch(Exception ex){
    ...
    private void initComponents(String editable, final String localfile,final String remotefile) {
        ...
        try{
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    jPanel1.setDividerLocation(RunnerRepository.getLayouts().
...
//         String dir = RunnerRepository.getRemoteEpIdDir();
//         String [] path = dir.split("/");
//         StringBuffer result = new StringBuffer();
//         if (path.length > 0) {
//             for (int i=0; i<path.length-1; i++){
...
//             }
//         }
//         String  [] vecresult = RunnerRepository.getRemoteFileContent(result.toString()+path[path.length-1]).split("\n");
