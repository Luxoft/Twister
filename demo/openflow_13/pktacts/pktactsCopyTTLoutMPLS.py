"""
<title>CopyTTLoutIPmpls</title>
<description>
    CopyTTLoutIPmpls to packet
...
class CopyTTLoutIPmpls(SimpleDataPlane):
    """
    CopyTTLoutIPmpls to packet
    """
    def runTest(self):
        self.logger.info("Running CopyTTLoutIPmpls test")
	...
	self.logger.info("Generate flowmod with action copy tll out")
        pkt = testutils.simple_tcp_packet(mpls_tags=[{'label': 50, 'ttl': 32}])
        ...
        vid_act = action.action_copy_ttl_out()
	...
	#TODO: a better verification of the incoming packet !!!
        self.assertTrue(p.ttl == 64, "Incoming packet should have mpls ttl 64")
