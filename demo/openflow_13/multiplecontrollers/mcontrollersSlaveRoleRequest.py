"""
<title>SlaveRoleRequest</title>
<description>A slave controller should be able to send role request messages
</description>
"""
...
class SlaveRoleRequest(MultipleController):

    """A slave controller should be able to send role request messages"""

    def runTest(self):

        self.logger.info("Running slave controller role request test ")
        ...
        self.logger.info("Sending role request")
        request = message.role_request()
        request.role = ofp.OFPCR_ROLE_NOCHANGE
        logMsg('logDebug',"Request to switch: ")
        logMsg('logDebug',request.show())
...
tc = SlaveRoleRequest(testbed=currentTB,ra_proxy=ra_service)
_RESULT = tc.run()
