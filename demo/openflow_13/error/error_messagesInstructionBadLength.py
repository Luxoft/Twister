"""
<title>InstructionBadLength</title>
<description>
    Switch must return error if instructions lenght is invalid
...
class InstructionBadLength(SimpleDataPlane):
    """
    Verify BAD_LEN error on instruction 
    """
    def runTest(self):
        self.logger.info("Running InstructionBadLength test")
	...
	inst.len = 2
        ...
        self.assertTrue(response.code==ofp.OFPFMFC_BAD_LEN,
                               'Error code is not OFPFMFC_BAD_LEN')

tc = InstructionBadLength()
_RESULT = tc.run()
