"""
<title>BadMatchLength</title>
<description>
    Verify a bad match length generate error
...
class BadMatchLength(SimpleDataPlane):
    """
    Verify flow_mod with a bad match lenght generate error
    """
    def runTest(self):
        self.logger.info("Running BadMatchLength test")
        self.logger.info("Insert flowmod and verify a bad length on match field generate error")
	...
	request.match.length = 2
        ...
        self.assertTrue(response.code==ofp.OFPBMC_BAD_LEN,
                               'Error code is not OFPBMC_BAD_LEN')
