"""
<title>MeterCountersBand</title>
<description>
    Insert meter_mod, flow_mod, send packets, verify band counters are incremented
...
class MeterCountersBand(SimpleDataPlane):
    """
    Insert meter_mod, flow_mod, send packets, verify band counters are incremented
    """
    def runTest(self):
        ...
        self.logger.info("Insert flowmod")
        ...
        for item in rv.stats:
            self.assertEqual(str(item.bands.packet_band_count), "1", "Flow_count must be 1")
            self.assertEqual(str(item.bands.byte_band_count), "100", "Flow_count must be 1")

    
tc = MeterCountersBand()
_RESULT = tc.run()
