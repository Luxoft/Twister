"""
<title>DurationNSECPerFlow</title>
<description>Verify duration in nano seconds counters per flow are
    incremented
</description>
"""
...
class DurationNSECPerFlow(SimpleDataPlane):

    """Verify duration in nano seconds counters per flow are
    incremented"""

    def runTest(self):

        self.logger.info("Running DurationNSECPerFlow test")
        ...
        self.logger.info("After sleep verify duration in nseconds counter increment in accordance")
        ...
        for item in response.stats:
            self.assertTrue(str(item.duration_nsec) != 0, "Duration should be different than 0")

    
tc = DurationNSECPerFlow()
_RESULT = tc.run()
