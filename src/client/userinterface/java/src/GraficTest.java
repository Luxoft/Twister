...
public class GraficTest extends JPanel {
		...
		if (ev.getButton() == 1) {
			...
			if (selected.size() > 0 && getItem(selected).getType() == 2) {
				if (getItem(selected).getSubItemsNr() > 0) {
					getItem(selected).setVisible(
							!(getItem(selected).getSubItem(0).isVisible()));
				}
	...
	/*
	 * update items location in tree view starting from suita
	 */
	public void updateLocations(Item suita) {
		...
		if (selected2.size() > 1) {
			...
			for (int i = index; i < Repository.getTestSuiteNr(); i++) {
				Repository.window.mainpanel.p2.sc.g.iterateThrough(
						Repository.getTestSuita(i), selected2);
				selected2 = null;
			}
		} else if (selected2.size() == 1) {
			for (int i = selected2.get(0); i < Repository.getTestSuiteNr(); i++) {
				Repository.window.mainpanel.p2.sc.g.iterateThrough(
						Repository.getTestSuita(i), null);
			}
		}
		y = 10;
	...
	/*
	 * calculates previous position for aligning on x
	 */
	public int calcPreviousPositions(Item item) {
		...
		if (item.getType() != 0) {
			...
			Item temp = getItem(pos);
			return temp.getLocation()[0]
					+ (int) (temp.getRectangle().getWidth() + 20);
		}
	}

	/*
	 * positions the item based on previous location
	 */
	public void positionItem(Item item) {
		if (item.getType() != 0) {
			...
			int x = calcPreviousPositions(item);
			item.setLocation(new int[] { x,
					(int) (y - 5 - item.getRectangle().getHeight()) });
		}
	}

	/*
	 * iterate through subitem of an item and position it
	 */
	...
	/*
	 * update scroll to adjust based on view dimension
	 */
	...
	/*
	 * handle drawing item based on item type and it's properties
	 */
	public void drawItem(Item item, Graphics g) {
		...
		if (item.getType() == 2) {
			...
			g.drawImage(Repository.getSuitaIcon(), (int) item.getRectangle()
					.getX() + 5, (int) item.getRectangle().getY() + 1, null);
		} else if (item.getType() == 1) {
			if (!item.isRunnable()) {
				g.setColor(Color.GRAY);
			}
			g.drawString(item.getName(), (int) item.getRectangle().getX() + 30,
					(int) item.getRectangle().getY() + 15);
			g.setColor(Color.BLACK);
			String value = item.getSubItem(0).getValue().toUpperCase();
			if (value.equals("FAIL")) {
				g.drawImage(Repository.getFailIcon(), (int) item.getRectangle()
						.getX() + 5, (int) item.getRectangle().getY() + 1, null);
			} else if (value.equals("PENDING")) {
				g.drawImage(Repository.getPendingIcon(), (int) item
						.getRectangle().getX() + 5, (int) item.getRectangle()
						.getY() + 1, null);
			} else if (value.equals("RUNNING")) {
				g.drawImage(Repository.getWorkingIcon(), (int) item
						.getRectangle().getX() + 5, (int) item.getRectangle()
						.getY() + 1, null);
			} else if (value.equals("SKIPPED")) {
				g.drawImage(Repository.getSkippedIcon(), (int) item
						.getRectangle().getX() + 5, (int) item.getRectangle()
						.getY() + 1, null);
			} else if (value.equals("STOPPED")) {
				g.drawImage(Repository.getStoppedIcon(), (int) item
						.getRectangle().getX() + 5, (int) item.getRectangle()
						.getY() + 1, null);
			} else if (value.equals("NOT EXECUTED")) {
				g.drawImage(Repository.getNotExecIcon(), (int) item
						.getRectangle().getX() + 5, (int) item.getRectangle()
						.getY() + 1, null);
			} else if (value.equals("TIMEOUT")) {
				g.drawImage(Repository.getTimeoutIcon(), (int) item
						.getRectangle().getX() + 5, (int) item.getRectangle()
						.getY() + 1, null);
			} else if (value.equals("WAITING")) {
				g.drawImage(Repository.getWaitingIcon(), (int) item
						.getRectangle().getX() + 5, (int) item.getRectangle()
						.getY() + 1, null);
			} else if (value.equals("PASS")) {
				g.drawImage(Repository.getPassIcon(), (int) item.getRectangle()
						.getX() + 5, (int) item.getRectangle().getY() + 1, null);
			} else {
			...
			g.drawImage(Repository.getPropertyIcon(), (int) item.getRectangle()
					.getX() + 2, (int) item.getRectangle().getY() + 1, null);
		...
		if (item.getEpId() != null) {
			...
			g.drawString(" - " + item.getEpId(), (int) (item.getRectangle()
					.getX() + item.getRectangle().getWidth() - 20), (int) (item
					.getRectangle().getY() + 18));
		}
	}
}
