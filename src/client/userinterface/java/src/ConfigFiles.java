...
public class ConfigFiles extends JPanel {
			...
			public void actionPerformed(ActionEvent ev) {
				if (!tname.getText().equals("")) {
					...
					JOptionPane.showMessageDialog(Repository.frame,
							"No file name given", "Filename missing",
		...
		loadXML.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ev) {
				try {
					...
					String config = (String) JOptionPane.showInputDialog(null,
							"Please select a config", "config", 1, null,
							configs, "Configs");
					if (config != null) {
						...
						if (doc != null) {
							...
							if (nodeLst.getLength() > 0) {
								...
								if (fstNm.item(0).getNodeValue().toString()
									...
									Repository.frame.mainpanel.p2 = new Panel2(
											Repository.applet);
									Repository.frame.mainpanel.setComponentAt(
											1, Repository.frame.mainpanel.p2);
									Repository.frame.mainpanel.p1.ep
											.refreshStructure();
									Repository.frame.mainpanel.p4.dbconfig
											.refresh();
									Repository
											.resetDBConf(
									...
									tdbfile.setText(Repository.REMOTEDATABASECONFIGPATH
											+ Repository.REMOTEDATABASECONFIGFILE);
									temailfile
											.setText(Repository.REMOTEEMAILCONFIGPATH
													+ Repository.REMOTEEMAILCONFIGFILE);
									thttpPort.setText(Repository
											.getHTTPServerPort());
									traPort.setText(Repository
											.getResourceAllocatorPort());
									tceport.setText(Repository
							...
							JOptionPane.showMessageDialog(Repository.frame,
									"Could not find Config tab", "WARNING",
									JOptionPane.WARNING_MESSAGE);
						}
					}
				} catch (Exception e) {
	...
	public JPanel addField(JTextField textfield, String text, int nr) {
		...
		if (Repository.logs.size() > 0) {
			textfield.setText(Repository.logs.get(nr));
		}
			...
			if (actionlistener == null) {
				b.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent ev) {
						...
						try {
							if (fileChooser.showOpenDialog(Repository.frame) == VFSJFileChooser.RETURN_TYPE.CANCEL) {
