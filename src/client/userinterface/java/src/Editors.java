...
public class Editors extends JFrame {
	...
	private void initComponents(Point p) {
		...
		remove.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ev) {
				...
				Repository.removeEditor(editorscombo.getSelectedItem()
						.toString());
		...
		tname.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent ev) {
				if (!tname.getText().equals(
						editorscombo.getSelectedItem().toString())) {
					String name = tname.getText();
					int caretpos = tname.getCaretPosition();
					int index = editorscombo.getSelectedIndex();
					if (defaultcheck.isSelected()) {
						Repository.setDefaultEditor(name);
					}
		...
		tcommand.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent ev) {
				saveTCommand(tcommand.getText(), editorscombo.getSelectedItem()
						.toString());
			}
		});
		...
		defaultcheck.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ev) {
				if (defaultcheck.isSelected()) {
					Repository.setDefaultEditor(editorscombo.getSelectedItem()
							.toString());
				} else {
					Repository.setDefaultEditor("Embedded");
				}
			}
		});
		...
		editorscombo.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent evt) {
				if (evt.getStateChange() == ItemEvent.SELECTED) {
					...
					if (Repository.getDefaultEditor().equals(
		...
		browse.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evnt) {
				...
				if (chooser.showOpenDialog(Editors.this) == JFileChooser.APPROVE_OPTION) {
					...
					saveTCommand(tcommand.getText(), editorscombo
		...
		tcommand.setText(Repository.getEditors().get(getEditors()[0])
				.getAsString());
										...
										layout.createParallelGroup(
												...
												.addGroup(
														layout.createSequentialGroup()
																...
																.addGap(10, 10,
																		10)))));
		layout.linkSize(SwingConstants.HORIZONTAL, new Component[] { remove,
				add });
	...
	/*
	 * change editor name name - new name element - the name to be removed
	 */
	public void saveTName(String name, String element) {
		Repository.addEditor(new String[] { name,
				Repository.getEditors().get(element).getAsString() });
		Repository.removeEditor(element);
	}

	/*
	 * define a new editor returns name and path of the new editor
	 */
	...
	/*
	 * get editors from repository as an array
	 */
