	...
	public void parseXML(Graphics g, boolean test) {
		...
		for (int m = 0; m < childsnr; m++) {
			...
			if (!test) {
				...
				boolean found = false;
				for (String s : Repository.logs) {
					if (s.equals(suitatemp.getEpId() + "_"
							+ Repository.logs.get(4))) {
						found = true;
						break;
					}
				}
				if (!found) {
					Repository.logs.add(suitatemp.getEpId() + "_"
							+ Repository.logs.get(4));
				}
			}
		}
		if (!test) {
			if (Repository.getSuiteNr() > 0) {
				...
				Repository.frame.mainpanel.p1.sc.g.updateLocations(Repository
						.getSuita(0));
				Repository.frame.mainpanel.p1.sc.g.repaint();
			}
		} else {
			if (Repository.getTestSuiteNr() > 0) {
				while (Repository.frame == null
						|| Repository.frame.mainpanel == null
						|| Repository.frame.mainpanel.p2 == null
						|| Repository.frame.mainpanel.p2.sc == null
						|| Repository.frame.mainpanel.p2.sc.g == null) {
					try {
						Thread.sleep(10);
					} catch (Exception e) {
						e.printStackTrace();
				...
				Repository.frame.mainpanel.p2.sc.g.updateLocations(Repository
						.getTestSuita(0));
