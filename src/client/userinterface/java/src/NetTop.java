...
public class NetTop extends JPanel {
	...
	private JPanel info;
	private JLabel switch1 = new JLabel("  Switch1", new ImageIcon(
			Repository.switche2), JLabel.RIGHT);
	private JLabel switch2 = new JLabel("  Switch2", new ImageIcon(
			Repository.switche2), JLabel.RIGHT);
	...
	public NetTop(int width, int height) {
		...
		info = new JPanel();
		info.setBounds(1150, 10, 430, 600);
			...
			public void run() {
				try {
					...
					while (Repository.run) {
						try {
							result = client.execute("ofStatistics",
									new Object[] {}) + "";
							...
							result = client.execute("ofDataPath",
									new Object[] {}) + "";
							setUpper(result);
							try {
								Thread.sleep(3000);
							...
							e.printStackTrace();
							System.out
									.println("could not retrieve net info from: "
											+ "http://"
											+ Repository.host
											+ ":"
											+ Repository.getCentralEnginePort());
	...
	public void paint(Graphics g) {
		...
		g.drawLine(1090, 0, 1090, getHeight());
		...
		g.drawLine(550, 145, 550, 420);

		g.drawLine(100, 145, 100, 420);
