...
public class Log extends JPanel {
	...
	/*
	 * interpret result from CE and update Log on screen accordingly
	 */
	...
	/*
	 * find previous occurrence of "toFind" in this log
	 */
	 ...
	 */
	public void highlite(String toFind, int rangeStart, int rangeEnd)
			throws Exception {
		HighlightPainter myHighlighter = new MyHighlightPainter(Color.RED);
		int index = textarea.getText(rangeStart, rangeEnd - rangeStart)
				.indexOf(toFind);
		...
		hilite.addHighlight(rangeStart + index,
				rangeStart + index + toFind.length(), myHighlighter);
		hilite.paint(textarea.getGraphics());
	}

	/*
	 * find next occurrence of "toFind" in this log
	 */
	...
	/*
	 * open filechooser and save log localy
	 */
	public void saveLog() {
	...
	public void clearLog() {
		clearScreen();
		try {
			String result = Repository.getRPCClient().execute("resetLog",
					new Object[] { log })
					+ "";
		} catch (Exception e) {
			e.printStackTrace();
