...
public class Device {
	...
	public void updateInfo() {
		...
		Repository.window.mainpanel.p4.getDut().tvendor.setText(vendor
				.toString());
		Repository.window.mainpanel.p4.getDut().tmodel
				.setText(model.toString());
		Repository.window.mainpanel.p4.getDut().tfamily.setText(family
				.toString());
		Repository.window.mainpanel.p4.getDut().tid.setText(id.toString());
		Repository.window.mainpanel.p4.getDut().tdescription
	...
	public void updatePropertys() {
		...
		if (Repository.window.mainpanel.p4.getDut().nodetemp1.getChildAt(
				Repository.window.mainpanel.p4.getDut().nodetemp1
						.getChildCount() - 1).isLeaf()) {
			while (Repository.window.mainpanel.p4.getDut().nodetemp1
					.getChildCount() > 6) {
		...
		for (int i = 0; i < properties.size(); i++) {
			...
			b.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent ev) {
					properties.remove(Repository.window.mainpanel.p4.getDut().properties
			...
			text1.addKeyListener(new KeyAdapter() {
				...
				public void keyReleased(KeyEvent ev) {
					properties.get(Repository.window.mainpanel.p4.getDut().properties
							.getComponentZOrder(text1) / 3)[0] = text1
							.getText();
					((DefaultMutableTreeNode) Repository.window.mainpanel.p4
							.getDut().nodetemp1
							.getChildAt(6 + (Repository.window.mainpanel.p4
									.getDut().properties
					...
					((DefaultTreeModel) Repository.window.mainpanel.p4.getDut().explorer.tree
							.getModel()).nodeChanged(Repository.window.mainpanel.p4
							.getDut().nodetemp1
							.getChildAt(6 + (Repository.window.mainpanel.p4
									.getDut().properties
				...
				public void keyReleased(KeyEvent ev) {
					...
					((DefaultMutableTreeNode) Repository.window.mainpanel.p4
							...
							.getChildAt(6 + (Repository.window.mainpanel.p4
									.getDut().properties
									.getComponentZOrder(text1) / 3)))
							.setUserObject(properties
									.get(Repository.window.mainpanel.p4
											.getDut().properties
		...
		Repository.window.mainpanel.p4.getDut().properties
				.setPreferredSize(new Dimension(Repository.window.mainpanel.p4
						.getDut().properties.getWidth(),
