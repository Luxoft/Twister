...
public class TabsReorder extends MouseInputAdapter{
    ...
    public void mouseReleased(MouseEvent ev){tab = -1;}
        
    public void mousePressed(MouseEvent ev) {tab = tabbed.getUI().tabForCoordinate(tabbed,ev.getX(), ev.getY());}

    public void mouseDragged(MouseEvent ev){
        if(tab == -1)return;
        int targetTabIndex = tabbed.getUI().tabForCoordinate(tabbed, ev.getX(), ev.getY());
        if(targetTabIndex != -1 && targetTabIndex != tab){
            boolean isForwardDrag = targetTabIndex > tab;
            tabbed.insertTab(tabbed.getTitleAt(tab), tabbed.getIconAt(tab), tabbed.getComponentAt(tab), tabbed.getToolTipTextAt(tab), isForwardDrag ? targetTabIndex+1 : targetTabIndex);
            tab = targetTabIndex;
