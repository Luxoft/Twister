...
public class DatabaseFrame extends JFrame{
        ...
        fieldsCombo.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if(fieldsCombo.getSelectedIndex()!=-1){
                    fileddescription.setText(desc[fieldsCombo.getSelectedIndex()]);}}});
    ...
    public void executeQuery(){
        ...
        try{config.setServerURL(new URL("http://"+Repository.host+":"+Repository.getCentralEnginePort()));}  //!!! NU uita sa updatezi portul in mesajul de eroare                          
        catch(Exception e){System.out.println("Could not conect to "+Repository.host+" :"+Repository.getCentralEnginePort());}
        XmlRpcClient client = new XmlRpcClient();
        client.setConfig(config);
        String query = userDefinition.getFieldID(); ;
        try{String result = (String)client.execute("runDBSelect",new Object[]{query});
