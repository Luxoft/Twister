...
public class ExplorerPanel extends JPanel {
    ...
    public ExplorerPanel(int x, int y, TreeDropTargetListener tdtl, boolean applet, ChannelSftp c){
        ...
        ds.createDefaultDragGestureRecognizer(this,DnDConstants.ACTION_COPY_OR_MOVE,
    ...
    public void treeClickReleased(MouseEvent ev){
        if(ev.isPopupTrigger()){refreshPopup(ev);}
            ...
            (tree.getModel().isLeaf(tree.getSelectionPath().getLastPathComponent()))){
                try{String thefile = tree.getSelectionPath().getParentPath().getLastPathComponent().toString()+
    ...
    public void refreshPopup(final MouseEvent ev){
        ...
        if((tree.getSelectionPaths().length==1) &&
        (tree.getModel().isLeaf(tree.getSelectionPath().getLastPathComponent())) &&
        ((editable.indexOf(".tcl")!=-1)||(editable.indexOf(".py")!=-1) ||
            ...
            item.addActionListener(new ActionListener(){
                public void actionPerformed(ActionEvent evnt){
                    try{new Editors(ev.getLocationOnScreen()).setVisible(true);}
                        ...
                        System.out.println("There was an error in opening editors configuration"+
    ...
    public void editWith(String editable){
        try{JsonObject editors = Repository.getEditors();
            ...
            int r = (Integer)CustomDialog.showDialog(p,JOptionPane.QUESTION_MESSAGE,
                                                    ...
                                                    ExplorerPanel.this,
                                                    "Please select an editor",null);
            if(r==JOptionPane.OK_OPTION){
                String ID = jComboBox1.getSelectedItem().toString();
                String remotefilename = tree.getSelectionPath().getPathComponent(tree.getSelectionPath().getPathCount()-2)+
                                        "/"+tree.getSelectionPath().getLastPathComponent();
                String localfilename = Repository.temp+Repository.getBar()+"Twister"+Repository.getBar()+
                                        tree.getSelectionPath().getLastPathComponent();
    ...
    public JPanel getEditorsPanel(JComboBox jComboBox1,String [] vecresult){
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jComboBox1, javax.swing.GroupLayout.PREFERRED_SIZE, 84,
    ...
    public void editTC(String editable){
        ...
        String remotefilename = tree.getSelectionPath().getPathComponent(tree.getSelectionPath().getPathCount()-2)+
                                "/"+tree.getSelectionPath().getLastPathComponent();
        String localfilename = Repository.temp+Repository.getBar()+"Twister"+
                                Repository.getBar()+tree.getSelectionPath().getLastPathComponent();
        ...
        saveuser.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev){
                try{FileWriter filewriter = new FileWriter(file);
                    ...
                    System.out.println("Could not save file localy : "+localfile);}}});
    ...
    public void sendFileToServer(File localfile, String remotefile){
        try{FileInputStream in = new FileInputStream(localfile);
            ...
            System.out.println("There was a problem in saving file "+
                                localfile.getName()+
                                " on hdd and uploading it to "+remotefile);}}
...
class Compare implements Comparator{   
    public int compare(Object emp1, Object emp2){ 
        return ((TreePath)emp1).getLastPathComponent().toString().
                compareToIgnoreCase(((TreePath)emp2).
