		...
		save.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ev) {
				...
				if (tpass.getPassword().length == 0) {
					CustomDialog
							.showInfo(JOptionPane.WARNING_MESSAGE, Emails.this,
									"Warning", "Warning, password not set");
				}
				try {
					File theone = new File(Repository.temp
							+ Repository.getBar()
							+ "Twister"
							+ Repository.getBar()
							+ "Config"
							+ Repository.getBar()
							+ new File(Repository.REMOTEEMAILCONFIGFILE)
									.getName());
					...
					try {
						...
						if (nodeLst.item(0).getChildNodes().getLength() > 0) {
							nodeLst.item(0).getChildNodes().item(0)
									.setNodeValue(check.isSelected() + "");
						} else {
							nodeLst.item(0)
									.appendChild(
											doc.createTextNode(check
													.isSelected() + ""));
						}
						nodeLst = doc.getElementsByTagName("SMTPPath");
						String SMTPPath = tipname.getText() + ":"
								+ tport.getText();
						if (nodeLst.item(0).getChildNodes().getLength() > 0) {
							nodeLst.item(0).getChildNodes().item(0)
									.setNodeValue(SMTPPath);
						} else {
							nodeLst.item(0).appendChild(
									doc.createTextNode(SMTPPath));
						}
						...
						if (tpass.getPassword().length != 0
							...
							if (nodeLst.item(0).getChildNodes().getLength() > 0) {
								...
								nodeLst.item(0).appendChild(
										doc.createTextNode(new String(tpass
												.getPassword())));
							}
						}
						...
						if (nodeLst.item(0).getChildNodes().getLength() > 0) {
							nodeLst.item(0).getChildNodes().item(0)
									.setNodeValue(tfrom.getText());
						} else {
							nodeLst.item(0).appendChild(
									doc.createTextNode(tfrom.getText()));
						}
						nodeLst = doc.getElementsByTagName("To");
						if (nodeLst.item(0).getChildNodes().getLength() > 0) {
							nodeLst.item(0).getChildNodes().item(0)
									.setNodeValue(emails.getText());
						} else {
							nodeLst.item(0).appendChild(
									doc.createTextNode(emails.getText()));
						}
						nodeLst = doc.getElementsByTagName("Message");
						if (nodeLst.item(0).getChildNodes().getLength() > 0) {
							nodeLst.item(0).getChildNodes().item(0)
									.setNodeValue(message.getText());
						} else {
							nodeLst.item(0).appendChild(
									doc.createTextNode(message.getText()));
						}
						nodeLst = doc.getElementsByTagName("Subject");
						if (nodeLst.item(0).getChildNodes().getLength() > 0) {
							nodeLst.item(0).getChildNodes().item(0)
									.setNodeValue(subject.getText());
						} else {
							nodeLst.item(0).appendChild(
									doc.createTextNode(subject.getText()));
						}
					} catch (Exception e) {
					...
					try {
						...
						try {
							Repository.c.cd(Repository.REMOTEEMAILCONFIGPATH);
						} catch (Exception e) {
				...
				if (saved) {
					CustomDialog.showInfo(JOptionPane.INFORMATION_MESSAGE,
							Emails.this, "Successfull",
							"File successfully saved");
				} else {
					CustomDialog.showInfo(JOptionPane.WARNING_MESSAGE,
							Emails.this, "Warning", "File could not be saved");
				}
			}
		});
		add(save);
