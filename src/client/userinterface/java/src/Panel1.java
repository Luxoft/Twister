...
//         System.out.println("Started Panel1 initialization: "+System.currentTimeMillis());
        ...
        item.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ev){
                try{ArrayList<Integer> temp = new ArrayList<Integer>();
                    ...
                    try{String ID = (String)JOptionPane.showInputDialog(sc.g,"Please select an EpID","EpID's", JOptionPane.INFORMATION_MESSAGE,null, vecresult,"EpID's");
                        theone.setEpId(ID);
        ...
        item.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ev){
                if(sc.g.getSelectedCollection().size()>1){
                    ...
                    for(int j=0;j<indexsize;j++){temp.add(new Integer(sc.g.getSelectedCollection().get(0)[j]));}
                    final Item theone = sc.g.getItem(temp,false);
                    if(theone.getPos().size()==1){//este pe nivelul 0
                    ...
                    else{int index = theone.getPos().get(theone.getPos().size()-1).intValue();//nu e pe nivelul 0
        ...
        item.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ev){
                ...
                int r = JOptionPane.showConfirmDialog(sc.g, message, "Property: value", JOptionPane.OK_CANCEL_OPTION);
                if(r == JOptionPane.OK_OPTION&&(!(name.getText()+value.getText()).equals(""))){
                    ArrayList<Integer> temp = new ArrayList<Integer>();
                    int indexsize = sc.g.getSelectedCollection().get(0).length;
                    for(int j=0;j<indexsize;j++){temp.add(new Integer(sc.g.getSelectedCollection().get(0)[j]));}
                    final Item theone = sc.g.getItem(temp,false);
                    ArrayList <Integer> indexpos3 = (ArrayList <Integer>)theone.getPos().clone();
                    indexpos3.add(new Integer(theone.getSubItemsNr()));
                    FontMetrics metrics = sc.g.getGraphics().getFontMetrics(new Font("TimesRoman", 0, 11));
                    ...
                    if(!theone.getSubItem(0).isVisible())property.setSubItemVisible(false);
        ...
        item.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ev){
                ...
                for(int j=0;j<indexsize;j++){temp.add(new Integer(sc.g.getSelectedCollection().get(0)[j]));}
                final Item theone = sc.g.getItem(temp,false);
                String name = JOptionPane.showInputDialog(sc.g,"Please enter the TC name","Suite Name",  JOptionPane.PLAIN_MESSAGE);
