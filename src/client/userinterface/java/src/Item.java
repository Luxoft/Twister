...
public class Item implements Cloneable {
	...
	public Item(String name, int type, int x, int y, int width, int height,
	...
	protected Item clone() {
		try {
			...
			for (int i = 0; i < clone.getSubItemsNr(); i++) {
				clone.subitems.set(i, clone.getSubItem(i).clone());
			}
			return clone;
	...
	public void setSubItemVisible(boolean visible) {
		...
		for (int i = 0; i < subitemsnr; i++) {
			getSubItem(i).setSubItemVisible(visible);
		}
	}
	...
	public void selectAll(Item item, boolean select) {
		...
		for (int i = 0; i < item.getSubItemsNr(); i++) {
			selectAll(item.getSubItem(i), select);
		}
	}
	...
	public void setLocation(int[] pos) {
		...
		if (type != 0) {
			checkrectangle.setLocation(pos[0] + 3, pos[1] + 3);
		}
	}

	public int[] getLocation() {
		return (new int[] { (int) rectangle.getLocation().getX(),
