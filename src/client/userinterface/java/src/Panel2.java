...
//         System.out.println("Started Panel2 initialization: "+System.currentTimeMillis());
            ...
            public void actionPerformed(ActionEvent ev){
                try{String status="";
                    if(play.getText().equals("Run")){
                        ...
                        status = (String)client.execute("setExecStatusAll",new Object[]{3});
                        play.setText("Pause");
                        play.setIcon(new ImageIcon(Repository.playicon));}
                    else if(play.getText().equals("Pause")){
                        status = (String)client.execute("setExecStatusAll",new Object[]{1});
                        play.setText("Resume");
                        play.setIcon(new ImageIcon(Repository.playicon));}}
                catch(Exception e){e.printStackTrace();}}});
        add(play);
        final JButton stop = new JButton("Stop",new ImageIcon(Repository.getStopIcon()));
        stop.setEnabled(false);
        stop.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ev){
                try{String status = (String)client.execute("setExecStatusAll",new Object[]{0});
                    play.setText("Run");
                    play.setIcon(new ImageIcon(Repository.playicon));
                    stoppushed = true;}
                catch(Exception e){e.printStackTrace();}}});
                    ...
                    try{XmlRpcClientConfigImpl config = new XmlRpcClientConfigImpl();
                        ...
                        while(Repository.run){
                            ...
                            if(!play.isEnabled()){
                                ...
                                stop.setEnabled(true);}
//                             System.out.println("Started updating statues: "+System.currentTimeMillis());
                            Object result1 = client.execute("getTestStatusAll",new Object[]{});
                            if(result1!=null){                                    
        ...
        new Thread(){
            public void run(){
                ...
                updateTabs();}}.start();
//         System.out.println("Finished Panel2 initialization: "+System.currentTimeMillis());
    ...
    public int addToExcel(WritableSheet sheet,Item element,int index,int columns){
        if(element.getType()==1){
            Label label;
            try{label = new Label(0, index,element.getFirstSuitaParent(true).getName());                
                sheet.addCell(label);
                label = new Label(1, index,element.getName());                
                sheet.addCell(label);
                label = new Label(2, index,element.getFirstSuitaParent(true).getEpId());                
                sheet.addCell(label);
    ...
    public int manageSubchildren(Item item, String[]statuses, int index){
        ...
        if(item.getType()==1&&statuses.length>index2){
            ...
            else if(statuses[index2].equals("7")||statuses[index2].equals("8"))item.getSubItem(0).setValue("timeout");
