...
public class Grafic extends JPanel {
	...
	public Grafic(TreeDropTargetListener tdtl, String user) {
		...
		setFocusable(true);
		if (!user.equals("")) {
			Repository.window.mainpanel.setTitleAt(0, (user.split("\\\\")[user
		...
		if (next != null && keypress != 2) {
			if (keypress != 1) {
				...
				if (next.getType() == 2 && next.getPos().size() == 1) {
					...
					if (userDefNr != Repository.window.mainpanel.p1.suitaDetails
							.getDefsNr()) {
		...
		if ((ev.getModifiers() & InputEvent.BUTTON1_MASK) != 0) {
			if (dragging) {
				...
				if (dragammount < 3) {
					...
					if (selected.size() > 0) {
						if (!(selectedcollection.size() < 2 && getItem(
								selected, false).isPrerequisite())) {
							handleDraggedItems();
						}
					}
				}
	...
	/*
	 * Dragged items method handles dragged items and puts them in a clone array
	 */
	...
	public void automaticScroll() {
		while (Repository.run) {
			if (scrolldown) {
				...
				Repository.window.mainpanel.p1.sc.pane.getVerticalScrollBar()
			...
			if (upper != null) {
				if (upper.getType() == 1) {
					...
					if ((upper.getSubItemsNr() > 0 && upper.getSubItem(0)
							.isVisible())) {
						if (upper.getSubItem(0).isPrerequisite()) {
							int index = upper.getSubItem(0).getPos()
									.get(upper.getPos().size() - 1).intValue();
							int position = upper.getSubItem(0).getPos().size() - 1;
							dropNextInLine(upper.getSubItem(0), upper, index,
									position);
						}
			...
			if (getItem(selected, false).getType() == 2) {
				if (getItem(selected, false).getSubItemsNr() > 0
					...
					int index = getItem(selected, false).getSubItem(0).getPos()
							.get(getItem(selected, false).getPos().size() - 1)
							.intValue();
					int position = getItem(selected, false).getSubItem(0)
							.getPos().size() - 1;
					dropNextInLine(getItem(selected, false).getSubItem(0),
							getItem(selected, false), index, position);
				}
	...
	/*
	 * checks if y is in the upper half of item
	 */
	...
	/*
	 * drops elements from clone as first lements in tree
	 */
	...
	/*
	 * drops elements from clone at first level under upper
	 */
	...
	/*
	 * drops elements from clone next in line after upper
	 */
	...
	/*
	 * drops elements from clone previous in line before upper
	 */
	...
	/*
	 * drops elements from clone on upper level from upper
	 */
	...
	/*
	 * drops elements from clone first in suite upper
	 */
	...
	/*
	 * positions dragging line inside suite
	 */
	...
	/*
	 * positions dragging line on suite
	 */
	...
	/*
	 * positions dragging line under suite
	 */
	...
	/*
	 * positions dragging line above tc
	 */
	...
	/*
	 * positions dragging line under item
	 */
	...
	/*
	 * positions dragging line after upper parent
	 */
	...
	/*
	 * positions dragging line after suite parent
	 */
	...
	/*
	 * positions dragging line after tc parent
	 */
	...
	/*
	 * position dragging line based on context
	 */
			...
			if (item.getType() == 2) {
				if (item.getRectangle().intersects(
					...
					if (item.getSubItemsNr() > 0
						...
						if (item.getSubItem(0).isPrerequisite()) {
		...
		if (dragscroll) {
			...
			if (Y
				...
				Repository.window.mainpanel.p1.sc.pane.getVerticalScrollBar()
					...
					- Repository.window.mainpanel.p1.sc.pane
							.getVerticalScrollBar().getValue() > Repository.window.mainpanel.p1.sc.pane
	...
	/*
	 * gets the item above X, Y position
	 */
	...
	/*
	 * returns last visible element after element at pos
	 */
	...
	/*
	 * prints this item and his subitems indices on screen
	 */
			...
			if (keypress == 0) {
				...
				if (selected.size() > 0) {
					...
					if (getItem(selected, false).getType() == 2
						...
						if (userDefNr != Repository.window.mainpanel.p1.suitaDetails
							...
							System.out
									.println("Warning, suite "
											...
											+ Repository.window.mainpanel.p1.suitaDetails
													.getDefsNr() + " fields");
							if (Repository.window.mainpanel.p1.suitaDetails
									.getDefsNr() < userDefNr) {
								temp.getUserDefs()
										.subList(
												Repository.window.mainpanel.p1.suitaDetails
						...
						try {
							for (int i = 0; i < Repository.window.mainpanel.p1.suitaDetails
									.getDefsNr(); i++) {
				...
				if (selected.size() > 0) {
					...
					if (theone1.isSelected()) {
						...
						for (int m = 0; m < selectedcollection.size(); m++) {
							if (Arrays
									.equals(selectedcollection.get(m), theone)) {
						...
						if (theone1.length > 1) {
							...
							if (Arrays.equals(temp1, temp2)) {
								...
								for (int i = second[second.length - 1]; i < first[first.length - 1] + 1; i++) {
									...
									int[] temporary = new int[parent
											.getSubItem(i).getPos().size()];
									for (int m = 0; m < temporary.length; m++) {
	...
	/*
	 * popup in case nothing is selected
	 */
	public void noSelectionPopUp(final MouseEvent ev) {
		 ...
		 * (chooser.showOpenDialog(Repository.window) ==
	...
	/*
	 * popup in case property is selected
	 */
	...
	public void redefineProp(Item prop) {
		...
		while (found) {
			...
			int resp = (Integer) CustomDialog.showDialog(p,
			...
			if (resp == JOptionPane.OK_OPTION
				...
				if (name.getText().equals("param") && name.isEnabled()) {
					CustomDialog
							.showInfo(JOptionPane.WARNING_MESSAGE, Grafic.this,
									"Warning",
									"'param' is used for parameters, please use a different name");
					continue;
				} else {
					found = false;
				}
	...
	/*
	 * name value panel created for adding props
	 */
	...
	/*
	 * popup in case there are multiple items selected
	 */
		...
		item.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ev) {
				String name = CustomDialog.showInputDialog(
						...
						JOptionPane.OK_CANCEL_OPTION, Grafic.this, "TC Name",
		...
		item.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ev) {
				String name = CustomDialog.showInputDialog(
						JOptionPane.QUESTION_MESSAGE,
						JOptionPane.OK_CANCEL_OPTION, Grafic.this,
						"Suite Name", "Please enter suite name").toUpperCase();
				FontMetrics metrics = getGraphics().getFontMetrics(
	...
	public void setPreRequisites(Item tc) {
		...
		if (firsttc.isPrerequisite()) {
			...
			int r = (Integer) CustomDialog.showDialog(message,
					JOptionPane.QUESTION_MESSAGE, JOptionPane.OK_CANCEL_OPTION,
					Grafic.this, "Warning", null);
	...
	public void addTCProp(Item tc) {
		...
		while (found) {
			...
			if (r == JOptionPane.OK_OPTION
				...
				ArrayList<Integer> indexpos3 = (ArrayList<Integer>) tc.getPos()
						.clone();
				...
				int width = metrics.stringWidth(name.getText() + ":  "
						+ value.getText()) + 38;
				Item property = new Item(name.getText(), 0, -1, -1, width, 20,
						indexpos3);
	...
	/*
	 * prompt user to add param to tc
	 */
	...
	/*
	 * panel displayed on twister tc setParam
	 */
	...
	/*
	 * remove selected items from Repository suites array
	 */
	public void removeSelected() {
		if (selectedcollection.size() > 0) {
			...
			for (int i = 0; i < selectednr; i++) {
				...
				if (theone.getType() == 0
						&& theone.getPos().get(theone.getPos().size() - 1) == 0) {
					theone.select(false);
				} else {
	...
	/*
	 * assign epid to item and its subsuites
	 */
	...
	/*
	 * select item based on his pos indices
	 */
	...
	/*
	 * get item with pos indices test - from mastersuites or suites
	 */
	...
	/*
	 * return if item or its subitem have been clicked
	 */
	...
	/*
	 * handle update locations from suita on
	 */
	...
	/*
	 * returns previous item from item half width locatiuon
	 */
	...
	/*
	 * positions item based on previous postion
	 */
	...
	/*
	 * used for calculating items positions iterates through subitems and
	 * positions them based previous location
	 */
	...
	/*
	 * handle paint item and subitems
	 */
	...
	/*
	 * handles drawing item based on item type
	 */
	...
	/*
	 * changes suites file name and sets name accordingly on tab
	 */
	...
	public int countSubtreeNr(int nr, Object child) {
		boolean cond;
		cond = Repository.window.mainpanel.p1.ep.tree.getModel().isLeaf(
				(TreeNode) child);
		...
		if (cond) {
			...
			int nr1 = Repository.window.mainpanel.p1.ep.tree.getModel()
					.getChildCount(((TreePath) child).getLastPathComponent());
			for (int j = 0; j < nr1; j++) {
				nr = countSubtreeNr(
						nr,
						Repository.window.mainpanel.p1.ep.tree.getModel()
								.getChild(
	...
	public void drop(int x, int y) {
		...
		if (max > 0) {
			for (int i = 0; i < max; i++) {
				boolean cond = Repository.window.mainpanel.p1.ep.tree
						.getModel().isLeaf(
								(TreeNode) Repository.window.mainpanel.p1.ep
										.getSelected()[i]
										.getLastPathComponent());
				if (cond) {
					String name = Repository.window.mainpanel.p1.ep
							.getSelected()[i].getPath()[Repository.window.mainpanel.p1.ep
							.getSelected()[i].getPathCount() - 2]
							+ "/"
							+ Repository.window.mainpanel.p1.ep.getSelected()[i]
									.getPath()[Repository.window.mainpanel.p1.ep
					...
					subtreeTC(
							(TreeNode) Repository.window.mainpanel.p1.ep.getSelected()[i]
	...
	public int subtreeTC(Object child, Item parent, int location) {
		...
		if (cond) {
			...
			int nr = Repository.window.mainpanel.p1.ep.tree.getModel()
					.getChildCount(((TreePath) child).getLastPathComponent());
			for (int j = 0; j < nr; j++) {
				location = subtreeTC(
						Repository.window.mainpanel.p1.ep.tree
								.getModel()
								.getChild(
										(TreeNode) ((TreePath) child)
												.getLastPathComponent(),
	...
	class AddSuiteFrame extends JFrame {
		...
		public AddSuiteFrame(final JComponent mainwindow, final Item suita,
				final int pos) {
