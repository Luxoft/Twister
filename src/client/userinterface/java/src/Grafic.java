...
public class Grafic extends JPanel{
    ...
    public Grafic(TreeDropTargetListener tdtl, String user){
        this.user=user;
        setFocusable(true);
        if(!user.equals("")){Repository.window.mainpanel.setTitleAt(0,(user.split("\\\\")[user.split("\\\\").length-1]).split("\\.")[0]);}
        add(p);
                ...
                if((ev.getModifiers() & InputEvent.BUTTON1_MASK) != 0){
                    if(dragging){handleDraggingLine(ev.getX(),ev.getY());}
                        ...
                        if(dragammount<3)dragammount++;
                            ...
                            if(selected.size()>0){
                                if(!(selectedcollection.size()<2&&getItem(selected,false).isPrerequisite())){//must not be prerequisite
                    ...
                    if(next!=null&&keypress!=2){
                        if(keypress!=1){
                            ...
                            if(next.getType()==2&&next.getPos().size()==1){
                                ...
                                if(userDefNr!=Repository.window.mainpanel.p1.suitaDetails.getDefsNr())System.out.println("Warning, suite "+next.getName()+" has "+userDefNr+" fields while in bd.xml are defined "+Repository.window.mainpanel.p1.suitaDetails.getDefsNr()+" fields");
                                try{for(int i=0;i<userDefNr;i++){Repository.window.mainpanel.p1.suitaDetails.getDefPanel(i).setDecription(next.getUserDef(i)[1]);}}
    ...
    public void handleMouseDroped(int mouseY){
        ...
        if(selected.size()==0){
            ...
            if(upper!=null){
                if(upper.getType()==1){//bagat sub tc
                    ...
                    if((upper.getSubItemsNr()>0&&upper.getSubItem(0).isVisible())){//suita este expanded sau nu are copii si trebuie bagat in ea
                        if(upper.getSubItem(0).isPrerequisite()){//primul element din suita e prerequisite, trebuie bagat dupa el
                    ...
                    int Y = mouseY;//se face ca upper sa devina parentul acestui prop si se copiaza metodele de la drop sub tc
            ...
            if(getItem(selected,false).getType()==2){//bagat in suita
                if(getItem(selected,false).getSubItemsNr()>0&&getItem(selected,false).getSubItem(0).isPrerequisite()){//primul element din suita este prerequisite trebuie bagat dupa el
                    int index = getItem(selected,false).getSubItem(0).getPos().get(getItem(selected,false).getPos().size()-1).intValue();
                ...
                dropFirstInSuita(getItem(selected,false));}//primul element nu e prerequisite, se poate baga pe prima pozitie
            else if(getItem(selected,false).getType()==1){//bagat in tc
                Item item = getItem(selected,false);
                boolean up = isUpperHalf(item, mouseY);
                if(up&&!item.isPrerequisite()){//System.out.println("deasupra tc"); e in partea de sus a tc si tc-ul nu e prerequisite
    ...
    public void handleDraggingLine(int X, int Y){
        ...
        if(item==null){
            ...
            if(item.getType()==2){//este o suita
                if(item.getRectangle().intersects(new Rectangle(0,Y-1,getWidth(),2))){//daca atinge suita
                    if(item.getSubItemsNr()>0&&item.getSubItem(0).isVisible()){//daca aceasta e expanded
                        if(item.getSubItem(0).isPrerequisite()){//daca primul element e prerequisite, trebuie bagat dupa el
                            lineUnderItem(item.getSubItem(0), X);}
                        else lineInsideSuita(item,X);}
                    else{lineOnSuita(item,X);}}//daca aceasta nu e expanded
                else if (item.getSubItemsNr()>0&&item.getSubItem(0).isVisible()){//daca nu atinge suita dar aceasta e expanded
                    if(item.getSubItem(0).isPrerequisite()){//daca primul element e prerequisite, trebuie bagat dupa el
                            lineUnderItem(item.getSubItem(0), X);}
                    else lineInsideSuita(item,X);}
                ...
                if(item.getRectangle().intersects(new Rectangle(0,Y-1,getWidth(),2))){//daca atinge tc-ul
                    if(item.isPrerequisite()){//daca tc-ul e prerequisite, trebuie bagat dupa el
                        lineUnderItem(item, X);}
                    else{//tc-ul nu e prerequisite, trebuie interpretata pozitia
        ...
        if(dragscroll){
            ...
            if(Y-Repository.window.mainpanel.p1.sc.pane.getVerticalScrollBar().getValue()<10){
                ...
                scrolldown = true;
                Repository.window.mainpanel.p1.sc.pane.getVerticalScrollBar().setValue(scrollvalue-10);}
            else if(Y-Repository.window.mainpanel.p1.sc.pane.getVerticalScrollBar().getValue()>Repository.window.mainpanel.p1.sc.pane.getSize().getHeight()-15){
                int scrollvalue = Repository.window.mainpanel.p1.sc.pane.getVerticalScrollBar().getValue();
                scrollup = true; 
                Repository.window.mainpanel.p1.sc.pane.getVerticalScrollBar().setValue(scrollvalue+10);}}}
                ...
                if(selected.size()>0){
                    ...
                    if(getItem(selected,false).getType()==2&&getItem(selected,false).getPos().size()==1){
                        ...
                        if(userDefNr!=Repository.window.mainpanel.p1.suitaDetails.getDefsNr()){
                            System.out.println("Warning, suite "+temp.getName()+" has "+userDefNr+" fields while in bd.xml are defined "+Repository.window.mainpanel.p1.suitaDetails.getDefsNr()+" fields");
                        ...
                        try{    
                            for(int i=0;i<Repository.window.mainpanel.p1.suitaDetails.getDefsNr();i++){
                                ...
                                Repository.window.mainpanel.p1.suitaDetails.getDefPanel(i).setDecription(temp.getUserDef(i)[1]);}}
                        catch(Exception e){e.printStackTrace();}}
...
/*         item = new JMenuItem("Open XML");
    ...
    public void redefineProp(Item prop){
        ...
        while(found){
            ...
            int resp = (Integer)CustomDialog.showDialog(p,JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, Grafic.this, "Property: value",null);
            if(resp == JOptionPane.OK_OPTION&&(!(name.getText()+value.getText()).equals(""))){
                if(name.getText().equals("param")&&name.isEnabled()){
                    CustomDialog.showInfo(JOptionPane.WARNING_MESSAGE, Grafic.this, "Warning", "'param' is used for parameters, please use a different name");
//                     JOptionPane.showMessageDialog(Grafic.this, "'param' is used for parameters, please use a different name");
                    continue;}
                else found = false;
...
//                 String name = JOptionPane.showInputDialog(Grafic.this,"Please enter the TC name","Suite Name",  JOptionPane.PLAIN_MESSAGE);
                String name = CustomDialog.showInputDialog(JOptionPane.QUESTION_MESSAGE, JOptionPane.OK_CANCEL_OPTION, Grafic.this, "TC Name", "Please enter the TC name");
                if(name!=null){
                ...
                public void actionPerformed(ActionEvent ev){
                    try{File f = new File(Repository.temp+System.getProperty("file.separator")+"Twister"+System.getProperty("file.separator")+"Epname.txt");
                        ...
                        try{
                            ...
                            int resp = (Integer)CustomDialog.showDialog(combo,JOptionPane.INFORMATION_MESSAGE,JOptionPane.OK_CANCEL_OPTION,Grafic.this,"Epname",null);
                            if(resp==JOptionPane.OK_OPTION){
                                String ID = combo.getSelectedItem().toString();
//                                 String ID = (String)JOptionPane.showInputDialog(Grafic.this,"Please select an Epname","Epnames", JOptionPane.INFORMATION_MESSAGE,null, vecresult,"Epname");
                                suita.setEpId(ID);
        ...
        item.addActionListener(new ActionListener(){
            ...
            String name = CustomDialog.showInputDialog(JOptionPane.QUESTION_MESSAGE, JOptionPane.OK_CANCEL_OPTION, Grafic.this, "Suite Name", "Please enter suite name").toUpperCase();
...
//             String name = JOptionPane.showInputDialog(Grafic.this,"Please enter the suite name","Suite Name",  JOptionPane.PLAIN_MESSAGE).toUpperCase();
    ...
    public void setPreRequisites(Item tc){
        boolean goon = true;
        Item firsttc = tc.getFirstSuitaParent(false).getSubItem(0);
        if(firsttc.isPrerequisite()){
            String message = "Suite already contains pre-requisite, overwrite?";
//             int r = JOptionPane.showConfirmDialog(Grafic.this, message, "Warning", JOptionPane.OK_CANCEL_OPTION);
            int r = (Integer)CustomDialog.showDialog(message, JOptionPane.QUESTION_MESSAGE, JOptionPane.OK_CANCEL_OPTION, Grafic.this, "Warning", null);
        ...
        if(goon){
            ...
            clone.add(theone2);
            removeSelected();
            dropFirstInSuita(theone2.getFirstSuitaParent(false));
            repaint();}}
...
//             int r = JOptionPane.showConfirmDialog(Grafic.this, message, "Property: value", JOptionPane.OK_CANCEL_OPTION);
            ...
            int r = (Integer)CustomDialog.showDialog(p,JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, Grafic.this, "Property: value",null);
...
//                     JOptionPane.showMessageDialog(Grafic.this, "'param' is used for parameters, please use a different name");
    ...
    public void setParam(Item tc){
        ...
        int resp = (Integer)CustomDialog.showDialog(p,JOptionPane.QUESTION_MESSAGE, JOptionPane.OK_CANCEL_OPTION, Grafic.this, "Property: value",null);
        if(resp == JOptionPane.OK_OPTION){
...
//         if(r == JOptionPane.OK_OPTION&&(!(name.getText()+value.getText()).equals(""))){
    ...
    public void removeSelected(){      
        if(selectedcollection.size()>0){
            ...
            for(int i=0;i<selectednr;i++){
                ...
                if(theone.getType()==0&&theone.getPos().get(theone.getPos().size()-1)==0)theone.select(false);//nu trebuie sa stearga prop de pe nivelul 0
    ...
    public int countSubtreeNr(int nr, Object child){
        ...
        if(cond){return nr+1;}
        else{int nr1 = Repository.window.mainpanel.p1.ep.tree.getModel().getChildCount(((TreePath)child).getLastPathComponent());
            for(int j=0;j<nr1;j++){
                nr = countSubtreeNr(nr,Repository.window.mainpanel.p1.ep.tree.getModel().getChild((TreeNode)((TreePath)child).getLastPathComponent(),j));}
        return nr;}}
    ...
    public void drop(int x, int y){
        ...
        if(max>0){
            for(int i=0;i<max;i++){
                boolean cond = Repository.window.mainpanel.p1.ep.tree.getModel().isLeaf((TreeNode)Repository.window.mainpanel.p1.ep.getSelected()[i].getLastPathComponent());//nu are copii                
                if(cond){
                    String name = Repository.window.mainpanel.p1.ep.getSelected()[i].getPath()[Repository.window.mainpanel.p1.ep.getSelected()[i].getPathCount()-2]+"/"+Repository.window.mainpanel.p1.ep.getSelected()[i].getPath()[Repository.window.mainpanel.p1.ep.getSelected()[i].getPathCount()-1];
                    ...
                    clone.add(newItem);}
                else{subtreeTC((TreeNode)Repository.window.mainpanel.p1.ep.getSelected()[i].getLastPathComponent(),null,0);}}
            handleMouseDroped(y);
            clone.clear();}}
    ...
    public int subtreeTC(Object child, Item parent, int location){
        ...
        if(cond){
            ...
            return location+1;}
        else{int nr = Repository.window.mainpanel.p1.ep.tree.getModel().getChildCount(((TreePath)child).getLastPathComponent());
            for(int j=0;j<nr;j++){
                location = subtreeTC(Repository.window.mainpanel.p1.ep.tree.getModel().getChild((TreeNode)((TreePath)child).getLastPathComponent(),j),parent,location);}
            return location;}}
    ...
    public void addSuiteFromButton(){
        if(selectedcollection.size()==0)new AddSuiteFrame(Grafic.this, null,0);
            ...
            if(selectedcollection.size()>1||getItem(temp,false).getType()!=2){
                CustomDialog.showInfo(JOptionPane.WARNING_MESSAGE, Grafic.this, "Warning", "Please select only one suite.");
