...
class flow_stats_entry(ofp_flow_stats):
    ...
    def unpack(self, binary_string):
        ...
        binary_string = self.match_fields.unpack(binary_string, bytes = self.match.length - 4)
        padding = roundup(self.match.length,8) - self.match.length
        if padding:
            binary_string = binary_string[padding:]
        ai_len = self.length - (OFP_FLOW_STATS_BYTES - 8) - roundup(self.match.length,8)
        if ai_len < 0:
...
class table_features_request(ofp_multipart_request, ofp_table_features):
    ...
    def __eq__(self, other):
        if type(self) != type(other): return False
        return (self.header == other.header and
                ofp_stats_request.__eq__(self, other))
...
class group_features_request(ofp_multipart_request):
    """
    Wrapper class for group features request
    """
    def __init__(self):
        self.header = ofp_header()
        ofp_multipart_request.__init__(self)
...
class group_features_reply(ofp_multipart_reply,ofp_group_features):
    """
    Wrapper class for group_features multipart reply
    """
    def __init__(self):
        self.header = ofp_header()
        ofp_multipart_reply.__init__(self)
	ofp_group_features.__init__(self)
        self.header.type = OFPT_MULTIPART_REPLY
        self.type = OFPMP_GROUP_FEATURES
    ...
    def pack(self, assertstruct=True):
        ...
        packed = self.header.pack()
        packed += ofp_multipart_reply.pack(self)
        packed += ofp_group_features.pack(self)
        return packed

    def unpack(self, binary_string):
        binary_string = self.header.unpack(binary_string)
        binary_string = ofp_multipart_reply.unpack(self, binary_string)
	binary_string = ofp_group_features.unpack(self,binary_string)
        return binary_string

    def __len__(self):
        length = len(self.header) + OFP_MULTIPART_REPLY_BYTES + OFP_GROUP_FEATURES_BYTES
        return length

    def show(self, prefix=''):
        outstr = prefix + "group_features_reply\n"
        outstr += prefix + "ofp header:\n"
        outstr += self.header.show(prefix + '  ')
        outstr += ofp_multipart_reply.show(self)
	outstr += ofp_group_features.show(self)
        return outstr

    def __eq__(self, other):
        if type(self) != type(other): return False
        return (self.header == other.header and
                ofp_multipart_reply.__eq__(self, other))
