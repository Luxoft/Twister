...
class Controller():
    ...
    def __init__(self,host="127.0.0.1",port=6633,ra_proxy=None,testbed=None,controller_name=None,channel_id=None):    
    ...
    def start(self,timeout=20):
        self.agentProxy.start_controller_channel(self.host,self.port)
        try:
            self.agentProxy.start_controller_channel(self.host,self.port)
        except:
    ...
    def connect(self,timeout=20):
        print "*************************"+str(self.agentProxy)
        print self.channel_id        
        rv=self.agentProxy.connect(self.channel_id,timeout)
    ...
    def transact(self,msg,timeout=20):
        ...
        res_msg = self.agentProxy.transact(self.channel_id,s_msg_xid,request,timeout)
        #decode reply
    ...
    def poll(self,exp_msg,timeout=20,xid=None):
        self.logger.info("Poll request")
        raw_msg=self.agentProxy.poll(self.channel_id, exp_msg,timeout)        
        if(raw_msg==''):
            return (None,None)
