...
def verify_flowstats(self,match=None,byte_count=None,packet_count=None):
    # Verify flow counters : byte_count and packet_count

    for i in range(0,60):
        stat_req = message.flow_stats_request()
        if match != None:
            stat_req.match_fields = match

	stat_req.out_port = ofp.OFPP_ANY
        stat_req.out_group = ofp.OFPG_ANY
	stat_req.table_id = ofp.OFPTT_ALL
        logging.info("Sending stats request")
        # TODO: move REPLY_MORE handling to controller.transact?
        response, pkt = self.controller.transact(stat_req,2)
...
# Generate Wildcard_All_Except_Ingress_port flow
    ...
    #Create a simple tcp packet and generate a matching flow entry.
...
def simple_eth_packet(pktlen=60,
    ...
    pkt = scapy.all.Ether(dst=dl_dst, src=dl_src, type=dl_type)
...
# Return all the port counters in the form a tuple 
    ...
    for obj in response.stats:
        ...
        dur_sec += obj.duration_sec
        dur_nsec += obj.duration_nsec 

    return (rx_pkts,tx_pkts,rx_byts,tx_byts,rx_drp,tx_drp,rx_err,tx_err,rx_fr_err,rx_ovr_err,rx_crc_err,collisions,tx_err,dur_sec,dur_nsec)
...
def simple_arp_packet(op1='is-at',
                      dl_src='00:06:07:08:09:0a',
                      dl_dst='00:06:07:08:09:0b',
                      p_src='192.168.0.1',
                      p_dst='192.168.0.2'):

    pkt = Ether(dst='00:1b:78:93:a6:16',src='90:2b:34:9a:7c:46') / ARP(psrc=p_src,pdst=p_dst,op=op1,hwsrc=dl_src,hwdst=dl_dst)
    return pkt
...
def simple_ipv6_packet(pktlen=100, 
    ...
    if (dl_vlan_enable):
        ...
        if(f_l != '0'):
            pkt = Ether(dst=dl_dst, src=dl_src)/ \
                IPv6(src=ip_src, dst=ip_dst, fl=f_l)
...
def simple_icmpv6_packet(pktlen=100, 
    ...
    if route_adv:
        ...
        if slldst_enabled:
            pkt = pkt/ \
            ICMPv6NDOptDstLLAddr(type=1, len=1, lladdr='66:6f:df:2d:7c:9b')
    else :
        pkt = pkt/ \
            ICMPv6EchoRequest()
    if (tcp_sport >0 and tcp_dport >0):
...
def simple_udp_packet(dl_dst='00:01:02:03:04:05',
                      dl_src='00:06:07:08:09:0a',
                      vlan_tags=[],  # {type,vid,pcp,cfi}  TODO type
                      mpls_tags=[],  # {type,label,tc,ttl} TODO type 
                      ip_src='192.168.0.1',
    ...
    pkt = pkt / IP(src=ip_src, dst=ip_dst, tos=ip_tos, ttl=ip_ttl) \
              / UDP(sport=tcp_sport, dport=tcp_dport)
...
def ag_flow_stats_get(parent, match_fields = None):
    """ Get the flow_stats from the switch
    Test the response to make sure it's really a flow_stats object
    """
    request = message.aggregate_stats_request()
    request.out_port = ofp.OFPP_ANY
    request.out_group = ofp.OFPG_ANY
    request.table_id = 0xff
    if match_fields != None:
